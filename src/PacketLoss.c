...
/**
 * Function for creating a 64-bit long identifier for each connection. 
 ...
 *     Dest port: 16 bits
 * NOTE: This implementation assumes source IP always starts with "192.168."
 *     and dest IP always starts with "10.0." so only last 16 bits are used 
 *     to identify IP addresses.
 * @param currConnection connection struct holding the connection IPs and ports
 */	
...
/**
 * Function for handling out-of-order packets from the trace stream. 
 */	
void storeOOSPacket(oos_avlNode** oosBSTRoot, struct packet currPacket) {
	...
	pkt->reset = currPacket.reset;
	pkt->connState = currPacket.connState;
	pkt->connID = currPacket.connID;

	// If connection is not already in oOS buffer, initialize heap and add 
	// key(connID) and value(heap):
...
/**
 * Function for updating the sequence number by checking the out-of-order packets buffer. 
 */	
int updateSeqNumsFromBuffer(avlNode** connBSTRoot, oos_avlNode** oosBSTRoot, 
								struct packet currPacket) {
	int connClosed = 0;
	if (oos_search(*oosBSTRoot, currPacket.connID) == NULL || 
			oos_search(*oosBSTRoot, currPacket.connID)->value->count == 0) return connClosed;
	...
	while(nextOOSSeqNum == search(*connBSTRoot, currPacket.connID)->value->seqNum 
			&& connOOSHeap->count) { // If the buffer contains the next packet
		search(*connBSTRoot, currPacket.connID)->value->seqNum = 
		...
		if (nextOOSPacket->fin || nextOOSPacket->reset || 
				(currPacket.payloadSize == 0 && currPacket.connState == 2))
		...
		//printf("updateSeqNumsFromBuffer() assigned new seqNum %d at time %.3f for conn %llx!\n", 
				// nextOOSSeqNum + nextOOSPacket->payloadSize + nextOOSPacket->fin, 
				// currPacket.timeStamp, search(*connBSTRoot, currPacket.connID)->key);
		do {
...
/**
 * Function for adding a node to a linked list of connections. 
 */	
...
/**
 * Function for updating the seq numbers of connections open. 
 * If the packet is out-of-order, it is stored. If closing the connection, 
 * connection is recorded in closed connections list and connection and 
 * associated outOfSeq packets are deleted.
 * @param line String array from a line of the trace file
 */	
int updateSeqNums(avlNode** connBSTRoot, oos_avlNode** oosBSTRoot, 
					struct packet currPacket) {
	int connClosed = 0;
	...
	// Else if packet is from open connection and matches next expected seq number
	} else if (search(*connBSTRoot, currPacket.connID) != NULL && 
				search(*connBSTRoot, currPacket.connID)->value->seqNum == currPacket.seqNum) {
		search(*connBSTRoot, currPacket.connID)->value->seqNum = 
			currPacket.seqNum + currPacket.payloadSize + currPacket.fin;
		search(*connBSTRoot, currPacket.connID)->value->timeStamp = 
		...
		connClosed = updateSeqNumsFromBuffer(connBSTRoot, oosBSTRoot, currPacket);
		if (currPacket.fin || currPacket.reset == 1 || 
				(currPacket.payloadSize == 0 && currPacket.connState == 2))
			connClosed = 1;
	
	//Else store in out-of-order buffer if packet is not in connections  
	//record (and not SYN) or has seq num greater than the current expected 
	//seq num, and is not a 0-bit packet (but include FIN and FIN ACK).
	} else if ((search(*connBSTRoot, currPacket.connID) == NULL || 
				search(*connBSTRoot, currPacket.connID)->value->seqNum < currPacket.seqNum) 
	...
	if (connClosed || (oos_search(*oosBSTRoot, currPacket.connID) != NULL && 
			oos_search(*oosBSTRoot, currPacket.connID)->value->count == 0)) {
		*oosBSTRoot = oos_deleteNode(*oosBSTRoot, currPacket.connID);
	}
	return connClosed;
}
...
//Initialize variables for recording packet contents
	char buff[16] = {0};
	int dataComplete = 1;
	struct packet currPacket;
...
//Initialize data structures for containing connections and out-of-order packet buffer
